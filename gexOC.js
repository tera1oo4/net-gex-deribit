...
// Конфигурация
const CONFIG = {
  WS_URL: 'wss://test.deribit.com/ws/api/v2', // тестнет
  // WS_URL: 'wss://www.deribit.com/ws/api/v2', // продакшн
...
async function authenticate() {
  try {
    ...
    console.log('⚠️  Продолжение без аутентификации (только публичные данные)');
  }
}
...
function saveChartHTML(gammaByExpiration, indexPrice) {
  ...
  // Подготавливаем данные для графиков по страйкам
  let strikeCharts = '';
  ...
  // Генерируем чекбоксы для фильтра
  let checkboxes = '';
...
<head>
    <meta charset="utf-8">
    <title>Gamma by Expiration - Deribit BTC Options</title>
...
<body>
    ...
    <script>
        ...
        // График 2: Колы vs Путы (группированные столбцы)
        ...
        // Функция для обновления объединённого графика
        ...
        // Инициализируем объединённый график при загрузке
        updateCombinedChart();
        
        // Графики по страйкам для каждой экспирации
        dates.forEach(expirationDate => {
            const expData = gammaByExpiration[expirationDate];
            const strikes = {};
  ...
  fs.writeFileSync('gamma_chart.html', html);
  console.log('\n✓ Графика сохранена в gamma_chart.html');
}
...
async function calculateGammaByExpiration(currency = 'BTC') {
  try {
    ...
    if (!indexPrice) {
      console.error('❌ Не удалось получить цену базового актива');
      return;
    }
    ...
    if (!bookSummary || bookSummary.length === 0) {
      console.error('❌ Не удалось получить рыночные данные');
      return;
    }
    
    // 4. Создаём карту market data
    const marketDataMap = {};
    ...
    sortedExpirations.forEach(expirationDate => {
      ...
      // Группируем по страйкам для консоли
      const strikeData = {};
    ...
    // 7. Сохраняем графику
    saveChartHTML(gammaByExpiration, indexPrice);
    
    return gammaByExpiration;
